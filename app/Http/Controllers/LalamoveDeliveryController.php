...
class LalamoveDeliveryController extends Controller
{
    public function quote(Request $request, LalamoveClient $lalamove): JsonResponse
    {
        ...
        try {
            $quotation = $lalamove->quotation($validated, ['quantity' => $validated['quantity'] ?? 1]);
    ...
    public function book(Request $request, Order $order, LalamoveClient $lalamove): RedirectResponse
    {
        ...
        if ($order->delivery_method !== 'lalamove') return back()->with('error', 'This order is not set for Lalamove delivery.');
        if ($order->lalamove_order_id) return back()->with('error', 'This order is already booked with Lalamove.');
    ...
    {
        $this->authorizeOrder($request, $order);
        if (!$order->lalamove_order_id) return back()->with('error', 'This order has no Lalamove delivery yet.');
    ...
    {
        $user = $request->user();
        abort_unless($user && ((int) $order->user_id === (int) $user->id || $order->isVisibleToPortalUser($user)), 403);
    }

    private function normalizePhone(?string $phone): string
    {
        $value = preg_replace('/[^0-9+]/', '', (string) $phone);
        if (str_starts_with($value, '09')) return '+63' . substr($value, 1);
        if (preg_match('/^9\d{9}$/', $value)) return '+63' . $value;
        return $value;
    }
}
