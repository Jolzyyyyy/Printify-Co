...
<section class="devx">
    ...
    <section class="devx-issue-grid">
        <article class="devx-panel"><h2>Recent Payment Issues</h2>@forelse($developer['recentPayments'] as $paymentOrder)<div class="devx-issue-line"><span><strong>{{ $paymentOrder->payment_reference ?: $paymentOrder->order_reference ?: '#'.$paymentOrder->id }}</strong><small>{{ $paymentOrder->payment_method ?: 'Payment' }} / {{ $paymentOrder->adminClient?->name ?? 'Unassigned' }}</small></span><b>PHP {{ number_format((float) $paymentOrder->total_price, 2) }}</b></div>@empty<div class="devx-empty">No payment issues for this range.</div>@endforelse</article>
        ...
        <article class="devx-panel"><h2>Recent Deliveries</h2>@forelse($developer['recentDeliveries'] as $deliveryOrder)<div class="devx-issue-line"><span><strong>{{ $deliveryOrder->delivery_tracking_number ?: $deliveryOrder->order_reference ?: '#'.$deliveryOrder->id }}</strong><small>{{ \Illuminate\Support\Str::headline($deliveryOrder->delivery_booking_status ?? $deliveryOrder->lalamove_status ?? 'Recorded') }}</small></span><b>{{ optional($deliveryOrder->delivery_booked_at ?? $deliveryOrder->created_at)->format('M d') }}</b></div>@empty<div class="devx-empty">No deliveries for this range.</div>@endforelse</article>
