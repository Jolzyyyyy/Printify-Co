@php
    ...
    $operationsCards = collect($developer['operationsCards'] ?? []);
    $reportsSecurityCards = collect($developer['reportsSecurityCards'] ?? []);
...
<style id="developer-dashboard-capstone-layout">
    ...
    .devx-card-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
    .devx-command-card{min-height:96px;border:1px solid #d8dee8;border-radius:8px;background:#fff;color:#111827;text-decoration:none;padding:13px;display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start}
    .devx-command-card:hover{border-color:#111827;background:#fff8f1}
    .devx-command-card small{display:block;font-size:9px;text-transform:uppercase;font-weight:900;color:#64748b;line-height:1.2}.devx-command-card strong{display:block;margin-top:5px;font-size:17px;font-weight:900;color:#111827;line-height:1.1}.devx-command-card em{display:block;margin-top:5px;font-size:11px;font-style:normal;font-weight:700;color:#64748b;line-height:1.35}
    ...
    @media(max-width:1300px){.devx-filter{grid-template-columns:1fr 1fr 1fr}.devx-kpi-grid,.devx-kpi-grid.four,.devx-card-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.devx-chart-grid,.devx-chart-grid.three,.devx-issue-grid{grid-template-columns:1fr}}
    @media(max-width:720px){.devx-head{display:grid}.devx-filter{grid-template-columns:1fr}.devx-kpi-grid,.devx-kpi-grid.four,.devx-card-grid{grid-template-columns:1fr}}
</style>
...
<section class="devx">
    ...
    <section class="devx-kpi-section">
        <h2 class="devx-section-title">Operations Command Center</h2>
        <div class="devx-card-grid">
            @forelse($operationsCards as $card)
                <a class="devx-command-card" href="{{ $card['url'] ?? '#' }}">
                    <span class="devx-kpi-icon {{ $card['tone'] ?? 'blue' }}"><i data-lucide="{{ $card['icon'] ?? 'circle' }}"></i></span>
                    <span>
            ...
            @empty
                <div class="devx-empty">No platform operations data available yet.</div>
            @endforelse
        </div>
    </section>

    <section class="devx-kpi-section">
        <h2 class="devx-section-title">Reports & Security</h2>
        <div class="devx-card-grid">
            @forelse($reportsSecurityCards as $card)
                <a class="devx-command-card" href="{{ $card['url'] ?? '#' }}">
                    <span class="devx-kpi-icon {{ $card['tone'] ?? 'blue' }}"><i data-lucide="{{ $card['icon'] ?? 'circle' }}"></i></span>
                    <span>
                        <small>{{ $card['label'] }}</small>
            ...
            @empty
                <div class="devx-empty">No report or security records available yet.</div>
            @endforelse
        </div>
    </section>
    ...
    <section class="devx-issue-grid">
        ...
        <article class="devx-panel"><h2>Reports & Security Activity</h2>@forelse($reportsSecurityCards->take(3) as $card)<a class="devx-issue-line" href="{{ $card['url'] ?? '#' }}" style="text-decoration:none;color:inherit"><span><strong>{{ $card['label'] }}</strong><small>{{ $card['note'] }}</small></span><b>{{ is_numeric($card['value']) ? number_format($card['value']) : $card['value'] }}</b></a>@empty<div class="devx-empty">No report or security activity yet.</div>@endforelse</article>
    </section>
</section>
