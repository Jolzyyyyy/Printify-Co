...
(function(){
  ...
  if(form)form.addEventListener('submit',async function(event){
    ...
    try{
      ...
      try{sessionStorage.setItem('printifyCheckoutReceiptRequested','true')}catch(e){}
      window.dispatchEvent(new CustomEvent('printify:e-receipt-submitted',{detail:{receipt:data.receipt||null,source:'submit'}}));
  ...
  fetch('{{ route('e-receipt.show') }}',{headers:{'Accept':'application/json'}}).then(function(response){return response.ok?response.json():null}).then(function(data){
    if(!data)return;
    if(data.request_complete){document.body.dataset.eReceiptRequested='true';try{sessionStorage.setItem('printifyCheckoutReceiptRequested','true')}catch(e){}window.dispatchEvent(new CustomEvent('printify:e-receipt-submitted',{detail:{receipt:data.receipt||null,source:'hydrate'}}))}
...
const initialReceiptRequested=@json((bool) session('checkout_e_receipt_request_id'))||document.body.dataset.eReceiptRequested==="true"||(function(){try{return sessionStorage.getItem("printifyCheckoutReceiptRequested")==="true"}catch(e){return false}})();
const state={items:[],promoCode:"",paymentVerified:false,receiptRequested:initialReceiptRequested,totals:{subtotal:0,discount:0,shipping:0,tax:0,total:0}};
...
function rememberReceiptRequest(){state.receiptRequested=true;document.body.dataset.eReceiptRequested="true";try{sessionStorage.setItem("printifyCheckoutReceiptRequested","true")}catch(e){}}
...
function imageMarkup(item){if(item.image)return '<img class="pfy-order-img" src="'+escapeHtml(item.image)+'" alt="'+escapeHtml(item.name)+'" loading="eager" decoding="sync" fetchpriority="high">';return '<div class="pfy-order-placeholder"><i class="fa-regular fa-file-lines"></i></div>'}
...
window.addEventListener("printify:e-receipt-submitted",function(event){rememberReceiptRequest();updateCheckoutState();if(event.detail&&event.detail.source==="submit")showToast("E-invoice request submitted. You can now select a payment method.","success")});
...
(function(){
  ...
  function renderBook(){
    var book=readBook();
    if(!book.length){
      list.innerHTML='<div class="pfy-address-empty"><strong>No saved address yet</strong></div>';
      return;
    }
