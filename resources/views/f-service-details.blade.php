...
const d=document,$=s=>d.querySelector(s),products={"text-only":{categoryKey:"doc",categoryTitle:"Document Printing",serviceName:"Text Only",title:"Text Only Printing",summaryTitle:"Text Only Printing",serviceId:"SRV-TXT-001",retailPrice:2,bulkPrice:1.5,bulkAt:50,unit:"sheets",previewType:"text",previewImage:"TXTONLY (B&W).png",previewImageCandidates:["TXTONLY (B&W).png","Text Only Printing.png","TEXT ONLY.png"],rating:"4.9",reviews:128},
...
<script>
/* FINAL SERVICE ID SYNC + SIDEBAR IMAGE-ONLY SAFETY */
(function(){
  function syncServiceId(){
    var target=document.getElementById('pdvServiceIdText');
    if(!target) return;
    var serviceId='SRV-TXT-001';
    try{
      var active=(window.state&&window.state.product)||null;
      if(active&&active.serviceId) serviceId=active.serviceId;
    }catch(e){}
    try{
      var latest=JSON.parse(localStorage.getItem('printifyActiveCheckout')||'null');
      if(latest&&latest.serviceId) serviceId=latest.serviceId;
    }catch(e){}
    /* Keep Text Only ID matching the requested mockup */
    if(serviceId==='DOC-TX-001') serviceId='SRV-TXT-001';
    target.textContent=serviceId;
  }

  var oldOpen=window.openPrintifyServiceDetail;
  if(typeof oldOpen==='function' && !oldOpen.__serviceIdColorPatch){
    var patched=function(){
      var result=oldOpen.apply(this,arguments);
      setTimeout(syncServiceId,20);
      setTimeout(syncServiceId,120);
      return result;
    };
    patched.__serviceIdColorPatch=true;
    window.openPrintifyServiceDetail=patched;
  }

  var oldUpdate=window.pdvUpdateOrder;
  if(typeof oldUpdate==='function' && !oldUpdate.__serviceIdColorPatch){
    var patchedUpdate=function(){
      var result=oldUpdate.apply(this,arguments);
      syncServiceId();
      return result;
    };
    patchedUpdate.__serviceIdColorPatch=true;
    window.pdvUpdateOrder=patchedUpdate;
  }

  document.addEventListener('DOMContentLoaded',syncServiceId);
  document.addEventListener('click',function(e){
    if(e.target.closest('#pdvThumbStack .pdv-thumb,#pdvPrintingCategory')) setTimeout(syncServiceId,80);
  });
