<?php ?>
<!-- =========================================================
  PRINTIFY & CO. - SERVICE DETAIL + CART PANEL (UPDATED IMAGE MAP)
...
<style id="requested-image-map-blank-state">
#serviceDetail .pdv-blank-preview{
  background:transparent!important;
  box-shadow:none!important;
  border:0!important;
  min-height:320px!important;
}
#serviceDetail .pdv-blank-summary{
  background:transparent!important;
  border:0!important;
  box-shadow:none!important;
}
...
Object.assign(products["lamination"],{previewImage:"LMNTN.png"});
Object.assign(products["spiral-binding"],{previewImage:"SPRL BNDNG.png"});
Object.assign(products["tarpaulin"],{previewImage:"TRPLN.png"});
Object.assign(products["sintra-board"],{previewImage:"SNTRBRD.png"});
...
/* ACCURATE SERVICE ROUTING MAP: fixes service cards that pass labels/categories/wrong URLs. */
const serviceAliasMap={
  "textonly":"text-only","textonlyprinting":"text-only","txtonly":"text-only","documenttextonly":"text-only","documentprintingtextonly":"text-only",
  "textwithimage":"text-image","textimage":"text-image","txtwi":"text-image","textwithimageprinting":"text-image",
  "imageonly":"image-only","imageonlyprinting":"image-only","io":"image-only",
  "documentprinting":"text-only","printing":"text-only",
  "photocopy":"photocopy","photocopyandscanning":"photocopy","photoandscanning":"photocopy","copy":"photocopy",
  "photocopytextonly":"photocopy-text-only","copytext":"photocopy-text-only","photocopytext":"photocopy-text-only",
  "photocopytextwithimage":"photocopy-text-image","photocopytextimage":"photocopy-text-image","copytextimage":"photocopy-text-image",
  "photocopyimageonly":"photocopy-image-only","copyimage":"photocopy-image-only",
  "scanning":"scanning","scan":"scanning","scanningtextonly":"scanning","scantext":"scanning",
  "scanningtextwithimage":"scanning-text-image","scanningtextimage":"scanning-text-image","scantextimage":"scanning-text-image",
  "scanningimageonly":"scanning-image-only","scanimage":"scanning-image-only",
  "photoandidservices":"id-photo","idandphotoservices":"id-photo","idphotoservices":"id-photo","photoidservices":"id-photo","idandphoto":"id-photo","idandphoto services":"id-photo",
  "idphoto":"id-photo","idpicture":"id-photo","idphotoprint":"id-photo",
  "passportvisa":"passport-visa","passportandvisa":"passport-visa","passport":"passport-visa","visa":"passport-visa",
  "singlephotoprint":"single-photo-print","photoprint":"single-photo-print","singlephoto":"single-photo-print","2r8r":"single-photo-print","2rto8r":"single-photo-print",
  "lamination":"lamination","laminationbinding":"lamination","laminationandbinding":"lamination","binding":"spiral-binding","spiralbinding":"spiral-binding",
  "largeformatprinting":"sintra-board","sintraboard":"sintra-board","sintraboardprinting":"sintra-board","tarpaulin":"tarpaulin",
  "customspecialprinting":"custom-layout","customlayout":"custom-layout","layout":"custom-layout","marketingcollateral":"marketing-collateral","marketing":"marketing-collateral","stickercut":"sticker-cut","sticker":"sticker-cut"
};
const categoryDefaultMap={doc:"text-only",document:"text-only",printing:"text-only",photo:"photocopy",copy:"photocopy",photocopy:"photocopy",scan:"scanning",scanning:"scanning",id:"id-photo",photoid:"id-photo",bind:"lamination",binding:"spiral-binding",largeformat:"sintra-board",special:"custom-layout",custom:"custom-layout"};
function compactKey(v){return String(v||"").toLowerCase().replace(/&/g,"and").replace(/[^a-z0-9]+/g,"").trim()}
function firstKnownServiceFromText(text){let c=compactKey(text);if(!c)return"";if(serviceAliasMap[c])return serviceAliasMap[c];let entries=Object.entries(serviceAliasMap).sort((a,b)=>b[0].length-a[0].length);let found=entries.find(([alias])=>alias.length>3&&c.includes(alias));return found?found[1]:""}
function resolveServiceKey(payload){
  if(payload&&typeof payload==="object"){
    for(let exactKey of ["serviceSlug","service_slug","detailSlug","detail_slug","serviceKey","service_key","optionSlug","option_slug","slug","key"]){
      if(payload[exactKey]){
        let direct=key(payload[exactKey]);
        if(products[direct])return direct;
        let mapped=firstKnownServiceFromText(payload[exactKey]);
        if(mapped&&products[mapped])return mapped;
      }
    }
    let values=[];
    ["serviceSlug","service_slug","serviceKey","service_key","slug","key","serviceName","service_name","name","title","summaryTitle","printingCategory","printing_category","categoryTitle","category_title","categoryKey","category_key","category"].forEach(k=>{if(payload[k])values.push(payload[k])});
    for(let v of values){let direct=key(v);if(products[direct])return direct;let found=firstKnownServiceFromText(v);if(found&&products[found])return found;let cat=categoryDefaultMap[compactKey(v)];if(cat&&products[cat])return cat;}
  }
  let direct=key(payload);if(products[direct])return direct;
  let found=firstKnownServiceFromText(payload);if(found&&products[found])return found;
  let cat=categoryDefaultMap[compactKey(payload)];if(cat&&products[cat])return cat;
  return "text-only";
}
function extractServicePayloadFromNode(node){
  let a=node?.closest?.("a[href]")||null,card=node?.closest?.("[data-service-slug],[data-service-key],[data-service],[data-slug],[data-category-key],[data-category],article,.service-card,.service-item,.product-card,.featured-service-card,.card")||node,p={};
  if(card?.dataset)Object.assign(p,card.dataset);
  if(a?.dataset)Object.assign(p,a.dataset);
  let heading=card?.querySelector?.("[data-service-title],.service-title,.product-title,.card-title,h1,h2,h3,h4,h5,h6");
  if(heading)p.title=heading.textContent.trim();
  let fullText=(card?.textContent||node?.textContent||"").replace(/view\s*full|view\s*all|selected|click|price|popular|sort\s*by/gi," ").trim();
  if(fullText)p.text=fullText;
  if(a){try{let url=new URL(a.getAttribute("href"),location.origin);p.hrefService=url.searchParams.get("service")||url.searchParams.get("slug")||url.pathname.split("/").filter(Boolean).pop()}catch(e){}}
  return p;
}
function resolveServiceKeyFromNode(node){
  let p=extractServicePayloadFromNode(node);
  /* Critical routing fix: visible card title must win over old/wrong href/data-service values. */
  let fromTitle=firstKnownServiceFromText(p.title||"");
  if(fromTitle&&products[fromTitle])return fromTitle;
  let fromText=firstKnownServiceFromText((p.text||"").replace(/view\s*full|view\s*all|selected|popular|sort\s*by|price|click/gi," "));
  if(fromText&&products[fromText])return fromText;
  let dataPayload=Object.assign({},p);delete dataPayload.hrefService;delete dataPayload.title;delete dataPayload.text;
  let fromData=resolveServiceKey(dataPayload);
  if(fromData&&products[fromData]&&(fromData!=="text-only"||firstKnownServiceFromText(Object.values(dataPayload).join(" "))))return fromData;
  if(p.hrefService)return resolveServiceKey(p.hrefService);
  return fromData||"text-only";
}
...
function selectOptions(p){
  let cat={
    doc:[["text-only","Text Only"],["text-image","Text with Image"],["image-only","Image Only"]],
    photo:[["photocopy","Photocopy"],["photocopy-text-only","Photocopy / Text Only"],["photocopy-text-image","Photocopy / Text with Image"],["photocopy-image-only","Photocopy / Image Only"],["scanning","Scanning / Text Only"],["scanning-text-image","Scanning / Text with Image"],["scanning-image-only","Scanning / Image Only"]],
    id:[["id-photo","ID Photo"],["passport-visa","Passport / Visa"],["single-photo-print","Single Photo Print"]],
    bind:[["lamination","Lamination"],["spiral-binding","Spiral Binding"]],
    largeformat:[["sintra-board","Sintra Board Printing"],["tarpaulin","Tarpaulin"]],
    special:[["custom-layout","Custom Layout"],["marketing-collateral","Marketing Collateral"],["sticker-cut","Sticker Cut"]]
  }[p.categoryKey]||[];
  el.cat.innerHTML=cat.map(o=>`<option value="${o[0]}">${o[1]}</option>`).join("");
  el.cat.value=state.productKey;
  let rows=variantRows(p),fallbackColors,fallbackPapers,fallbackFinish;
  if(state.productKey==="id-photo"){
    fallbackColors=[["Full Color","Full Color"],["B&W","B&W"],["Matte Tone","Matte Tone"]];
    fallbackPapers=[["Package A","Package A"],["Package B","Package B"],["Package C","Package C"],["Package D","Package D"],["Package E","Package E"]];
    fallbackFinish=[["Basic Retouch","Basic Retouch"],["Background Cleanup","Background Cleanup"],["Glossy Finish","Glossy Finish"],["Soft Copy Included","Soft Copy Included"]];
  }else if(state.productKey==="passport-visa"){
    fallbackColors=[["Full Color","Full Color"],["B&W","B&W"],["Matte Tone","Matte Tone"]];
    fallbackPapers=[["Passport","Passport"],["Visa","Visa"]];
    fallbackFinish=[["Basic Retouch","Basic Retouch"],["Background Cleanup","Background Cleanup"],["Glossy Finish","Glossy Finish"],["Soft Copy Included","Soft Copy Included"]];
  }else if(state.productKey==="single-photo-print"){
    fallbackColors=[["Full Color","Full Color"],["B&W","B&W"],["Matte Tone","Matte Tone"]];
    fallbackPapers=[["2R","2R"],["3R","3R"],["4R","4R"],["5R","5R"],["6R","6R"],["7R","7R"],["8R","8R"]];
    fallbackFinish=[["Glossy Finish","Glossy Finish"],["Matte Finish","Matte Finish"],["Borderless Cut","Borderless Cut"],["Standard Print","Standard Print"]];
  }else{
    fallbackColors=["largeformat","special"].includes(p.categoryKey)?[["Standard Production","Standard Production"],["Premium Production","Premium Production"]]:[["B&W","B&W"],["Partial Color","Partial Color"],["Full Color","Full Color"]];
    fallbackPapers=p.categoryKey==="largeformat"?[["A4 (8.27 x 11.69)","A4 (8.27 x 11.69)"],["3x4 ft","3x4 ft"]]:[["Short (8.5 x 11)","Short (8.5 x 11)"],["A4 (8.27 x 11.69)","A4 (8.27 x 11.69)"],["Legal (8.5 x 14)","Legal (8.5 x 14)"]];
    fallbackFinish=[["Standard Service","Standard Service"]];
  }
  setSelectOptions(el.color,uniqueOptions(rows,"colorMode",fallbackColors));
  setSelectOptions(el.paper,uniqueOptions(rows,"productSize",fallbackPapers));
...
const requestedImageMap={
  "text-only":{BW:"TXTONLY (B&W).png",FC:"TXTONLY (FC).png",PC:"TXTONLY (PC).png"},
  "text-image":{BW:"TXTWI (B&W).png",FC:"TXTWI (FC).png",PC:"TXTWI (PC).png"},
  "image-only":{BW:"IO (B&W).png",FC:"IO (FC).png",PC:"IO (PC).png"},
  "photocopy":{BW:"PHOTOC (B&W).png",FC:"PHOTOC (FC).png",PC:"PHOTOC (PC).png"},
  "photocopy-text-only":{BW:"PHOTOC (B&W).png",FC:"PHOTOC (FC).png",PC:"PHOTOC (PC).png"},
  "photocopy-text-image":{BW:"PHOTOC (B&W).png",FC:"PHOTOC (FC).png",PC:"PHOTOC (PC).png"},
  "photocopy-image-only":{BW:"PHOTOC (B&W).png",FC:"PHOTOC (FC).png",PC:"PHOTOC (PC).png"},

  /* LAMINATION & BINDING */
  "lamination":{BW:"LMNTN.png",FC:"LMNTN.png",PC:"LMNTN.png"},
  "spiral-binding":{BW:"SPRL BNDNG.png",FC:"SPRL BNDNG.png",PC:"SPRL BNDNG.png"},
...
function mediaFileFor(serviceKey,opts={}){
  let sk=resolveServiceKey(serviceKey||state.productKey),color=opts.color||txt(el.color)||"B&W",paper=opts.paper||txt(el.paper)||"",finish=opts.finish||txt(el.svc)||"",paperN=normMedia(paper),finishN=normMedia(finish);
  if(requestedImageMap[sk])return requestedImageMap[sk][colorCode(color)]||"";
  if(sk==="id-photo")return "PCKG A-E.png";
  if(sk==="single-photo-print")return "SP 2R-8R.png";
  if(sk==="passport-visa")return (paperN.includes("visa")||finishN.includes("visa"))?"VISA.png":"PSSPRT.png";
  return "";
}
...
window.pdvUpdateOrder=function(){let o=calc(),p=state.product,unit=o.qty>1?p.unit:p.unit.replace(/s$/," ").trim(),serviceId=document.getElementById("pdvServiceIdText");refreshMedia();el.sumTitle.textContent=p.summaryTitle;el.sumMeta.textContent=txt(el.paper)+" - "+txt(el.color);el.sumQty.textContent=o.qty?o.qty+" "+unit+" \u2022 "+txt(el.svc):"Quantity not selected \u2022 "+txt(el.svc);el.retail.textContent=money(o.retailPrice);el.bulk.textContent=money(o.bulkPrice);el.total.textContent=money(o.total);if(serviceId)serviceId.textContent=o.serviceId;el.mode.textContent=!o.qty?"Select quantity using the \u2212 / + buttons or type the number to calculate total amount.":o.isBulk?`Bulk pricing is active because quantity reached ${p.bulkAt} ${p.unit}.`:`Retail pricing applies below ${p.bulkAt} ${p.unit}. Click Bulk Price to jump to bulk quantity.`;el.retailCard.classList.toggle("is-active",!o.isBulk);el.bulkCard.classList.toggle("is-active",o.isBulk);el.bulkHint.textContent=`Click for ${p.bulkAt}+ ${p.unit.replace(/s$/,"")}`;el.checkout.disabled=false;el.checkout.classList.toggle("is-disabled",!state.selectedFile||!o.qty);el.note.querySelector("small").textContent=state.selectedFile?"File uploaded. Please confirm that this is the correct file before checkout.":"Checkout will be allowed only after a file is uploaded."};
...
/* Accurate card routing: even if an old card href says custom-layout, the visible card title wins. */
d.addEventListener("click",e=>{
  let t=e.target.closest("[data-service-slug],[data-service-key],[data-service],[data-slug],[data-category-key],a[href*=\"service-details\"],a[href*=\"service=\"],.service-card,.service-item,.product-card,.featured-service-card");
  if(!t||t.closest("#serviceDetail")||t.closest("#pfyCartPanel"))return;
  let serviceKey=resolveServiceKeyFromNode(t);
  if(!serviceKey||!products[serviceKey])return;
  if(t.matches("a[href*=\"service-details\"],a[href*=\"service=\"],[data-service-slug],[data-service-key],[data-service],[data-slug],.service-card,.service-item,.product-card,.featured-service-card")){
    e.preventDefault();
    window.openPrintifyServiceDetail({serviceSlug:serviceKey},true);
  }
})}
...
body.front-route-service-details:not(.service-detail-hydrated) #serviceDetail{visibility:hidden!important}
</style>
...
(function(){
  ...
  if(document.body.classList.contains('service-detail-hydrated'))revealServiceDetail();
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(revealServiceDetail,700)},{once:true});
  else setTimeout(revealServiceDetail,700);
})();
</script>
  ...
  function backendPayload(items){
    return (items||[]).map(function(item){
      ...
      return {
        ...
        unit:raw.unit||'pcs',
        image_path:item.image||raw.image_path||raw.image||raw.previewImage||null,
...
(function(){
  ...
  var SERVICE_ROUTE_MAP={
    "textonly":"text-only","textonlyprinting":"text-only","txtonly":"text-only","txtonlybw":"text-only","txtonlyfc":"text-only","txtonlypc":"text-only",
    "textwithimage":"text-image","textimage":"text-image","txtwi":"text-image","txtwibw":"text-image","txtwifc":"text-image","txtwipc":"text-image",
    "imageonly":"image-only","imageonlyprinting":"image-only","io":"image-only","iobw":"image-only","iofc":"image-only","iopc":"image-only",
    "documentprinting":"text-only","document":"text-only",
    "photocopy":"photocopy","photocopyandscanning":"photocopy","copy":"photocopy",
    "photocopytextonly":"photocopy-text-only","copytext":"photocopy-text-only",
    "photocopytextwithimage":"photocopy-text-image","photocopytextimage":"photocopy-text-image","copytextimage":"photocopy-text-image",
    "photocopyimageonly":"photocopy-image-only","copyimage":"photocopy-image-only",
    "scanning":"scanning","scan":"scanning","scanningtextonly":"scanning","scantext":"scanning",
    "scanningtextwithimage":"scanning-text-image","scanningtextimage":"scanning-text-image","scantextimage":"scanning-text-image",
    "scanningimageonly":"scanning-image-only","scanimage":"scanning-image-only",
    "photoandidservices":"id-photo","idandphotoservices":"id-photo","idandphoto":"id-photo","idphotoservices":"id-photo","photoidservices":"id-photo","photoid":"id-photo","idphoto":"id-photo","idpicture":"id-photo","idphotoprint":"id-photo",
    "passportvisa":"passport-visa","passportandvisa":"passport-visa","passport":"passport-visa","visa":"passport-visa",
    "singlephotoprint":"single-photo-print","singlephoto":"single-photo-print","photoprint":"single-photo-print","2r8r":"single-photo-print","2rto8r":"single-photo-print",
    "lamination":"lamination","laminationbinding":"lamination","laminationandbinding":"lamination","binding":"spiral-binding","spiralbinding":"spiral-binding",
    "largeformatprinting":"sintra-board","sintraboard":"sintra-board","sintraboardprinting":"sintra-board","tarpaulin":"tarpaulin",
    "customspecialprinting":"custom-layout","customlayout":"custom-layout","layout":"custom-layout","marketingcollateral":"marketing-collateral","marketing":"marketing-collateral","stickercut":"sticker-cut","sticker":"sticker-cut"
  };

  function compact(v){return String(v||"").toLowerCase().replace(/&/g,"and").replace(/[^a-z0-9]+/g,"").trim();}
  function byText(v){var c=compact(v);if(!c)return"";if(SERVICE_ROUTE_MAP[c])return SERVICE_ROUTE_MAP[c];var pairs=Object.keys(SERVICE_ROUTE_MAP).sort(function(a,b){return b.length-a.length;});for(var i=0;i<pairs.length;i++){if(pairs[i].length>3&&c.indexOf(pairs[i])>-1)return SERVICE_ROUTE_MAP[pairs[i]];}return"";}
  function slug(v){return String(v||"").toLowerCase().trim().replace(/&/g,"and").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");}
  function routeOfNode(node){
    var card=node.closest('[data-service-slug],[data-service-key],[data-service],[data-slug],.service-card,.service-item,.product-card,.featured-service-card,.card,article')||node;
    var a=node.closest('a[href]')||(card.querySelector&&card.querySelector('a[href]'));
    var exactSlug='';
    if(card&&card.classList&&card.classList.contains('pfsvc-card')&&card.dataset){
      exactSlug=card.dataset.serviceSlug||card.dataset.serviceKey||card.dataset.service||card.dataset.slug||'';
      if(exactSlug){
        var exactDirect=slug(exactSlug), exactMapped=SERVICE_ROUTE_MAP[compact(exactSlug)]||byText(exactSlug);
        if(exactMapped)return exactMapped;
        if(exactDirect)return exactDirect;
      }
    }
    var values=[];
    [card,node,a].forEach(function(x){
      if(x&&x.dataset){
        ['serviceSlug','serviceKey','service','slug','title','name','categoryKey','category'].forEach(function(k){
          if(x.dataset[k])values.push(x.dataset[k]);
        });
      }
    });
    var h=card.querySelector&&card.querySelector('[data-service-title],.service-title,.product-title,.card-title,h1,h2,h3,h4,h5,h6');
    var heading=h?h.textContent:'';
    var fullText=card.textContent?card.textContent.replace(/view\s*full|view\s*all|selected|popular|sort\s*by|price|click/gi,' '):'';
    var hrefValue='';
    if(a){
      try{
        var u=new URL(a.getAttribute('href'),location.origin);
        hrefValue=u.searchParams.get('service')||u.searchParams.get('slug')||u.pathname.split('/').filter(Boolean).pop()||'';
      }catch(e){}
    }

    /* Critical routing fix:
       1) visible card heading first (ID Photo, Text Only, Tarpaulin, etc.)
       2) visible card text second
       3) data-* attributes third
       4) href only as last fallback, because many old cards have href=?service=sintra-board */
    var fromHeading=byText(heading);
    if(fromHeading)return fromHeading;
    var fromText=byText(fullText);
    if(fromText)return fromText;
    for(var i=0;i<values.length;i++){
      var raw=values[i],direct=slug(raw),mapped=byText(raw);
      if(SERVICE_ROUTE_MAP[compact(raw)])return SERVICE_ROUTE_MAP[compact(raw)];
      if(mapped)return mapped;
      if(direct&&byText(direct))return byText(direct);
    }
    return byText(hrefValue)||slug(hrefValue)||'';
  }
  function go(serviceKey,scroll){
    if(!serviceKey)return;
    try{sessionStorage.setItem('selectedPrintifyService',JSON.stringify({serviceSlug:serviceKey,source:'accurate-hard-fix'}));}catch(e){}
    var url='/service-details?service='+encodeURIComponent(serviceKey);
    if(/\/service-details?\/?$/i.test(location.pathname)&&typeof window.openPrintifyServiceDetail==='function'){
      history.replaceState(history.state||{},'',url);
      window.openPrintifyServiceDetail({serviceSlug:serviceKey},scroll!==false);
      setTimeout(function(){window.openPrintifyServiceDetail({serviceSlug:serviceKey},false);},80);
    }else{
      location.href=url;
    }
  }
  ...
  function repairServiceLinks(){
    var selector='[data-service-slug],[data-service-key],[data-service],[data-slug],a[href*="service-details"],a[href*="service="],.service-card,.service-item,.product-card,.featured-service-card';
    document.querySelectorAll(selector).forEach(function(node){
      if(node.closest&&node.closest('#serviceDetail,#pfyCartPanel'))return;
      var k=routeOfNode(node);
      if(!k)return;
      var card=node.closest&&node.closest('.service-card,.service-item,.product-card,.featured-service-card,article,[data-service-slug],[data-service-key],[data-service],[data-slug]');
      var targets=[];
      if(node.matches&&node.matches('a[href*="service-details"],a[href*="service="]'))targets.push(node);
      if(card&&card.querySelectorAll)targets=targets.concat(Array.prototype.slice.call(card.querySelectorAll('a[href*="service-details"],a[href*="service="]')));
      targets.forEach(function(a){
        try{a.setAttribute('href','/service-details?service='+encodeURIComponent(k));}catch(e){}
      });
      if(card&&card.dataset)card.dataset.serviceSlug=k;
    });
  }

  document.addEventListener('click',function(e){
    var t=e.target.closest('[data-service-slug],[data-service-key],[data-service],[data-slug],a[href*="service-details"],a[href*="service="],.service-card,.service-item,.product-card,.featured-service-card');
    if(!t||t.closest('#serviceDetail')||t.closest('#pfyCartPanel'))return;
    var k=routeOfNode(t);
    if(!k)return;
    e.preventDefault();
  ...
  function hydrateFromUrl(){
    if(!/\/service-details?\/?$/i.test(location.pathname))return;
    var q=new URLSearchParams(location.search).get('service');
    var k=byText(q)||slug(q);
    if(k&&typeof window.openPrintifyServiceDetail==='function')go(k,false);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){repairServiceLinks();hydrateFromUrl();},{once:true});else{repairServiceLinks();hydrateFromUrl();}
  window.addEventListener('pageshow',function(){repairServiceLinks();hydrateFromUrl();});
})();
</script>
